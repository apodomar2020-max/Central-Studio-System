-- Migration 0108: Category-based General Class walk-in pricing (Adults / Teens / Kids)
--
-- Renumbered from 0106 -> 0108 during integration with origin/main: both this
-- branch and origin/main independently claimed slot 0106 off the same 0105
-- base (origin used it for 0106_notification_campaign_audience_segments, and
-- followed it with 0107_notification_delivery_logs_status_created_at_index).
-- This migration's own SQL is unchanged — only its filename, journal tag, and
-- position in the applied sequence moved. It remains purely additive to
-- `classes`/`class_pricing_settings` and independent of the notification
-- migrations it now applies after.
--
