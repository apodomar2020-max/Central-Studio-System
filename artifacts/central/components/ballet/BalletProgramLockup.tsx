...
export function BalletProgramLockup({
  ...
  const iconSize = isHero ? 76 : 40;
  const dividerHeight = isHero ? 76 : 40;

  return (
    <View style={[styles.lockup, isHero ? styles.heroLockup : styles.cardLockup, style]}>
      <BallerinaShoesIcon size={iconSize} color={color} />
      <View style={[
        styles.divider,
        { height: dividerHeight, backgroundColor: color === "#FFFFFF" ? "rgba(255,255,255,0.86)" : color },
      ]} />
      <Text style={[isHero ? styles.heroTitle : styles.cardTitle, { color }]}>{`${firstLine}\n${secondLine}`}</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  heroTitle: {
    ...
    marginBottom: -iosCapGuard(44, 40),
  },
  cardTitle: {
    fontSize: 25,
    fontFamily: "Anton_400Regular",
    ...
    lineHeight: 23,
    letterSpacing: 0.3,
    ...iosDisplayTextStyle(25, 23),
    marginBottom: -iosCapGuard(25, 23),
  },
});
