...
import { z } from "zod";
import { Plus, Pencil, Trash2, GripVertical, CheckCircle2, XCircle, Save, Music2, Play, Square, AlertTriangle } from "lucide-react";
...
interface OverCapacityOccurrence {
  scheduleId: number;
  classId: number;
  classTitle: string;
...
type WorkerHealthStatus = "online" | "stale" | "unknown";

interface ClassReminderStatus {
...
export default function SettingsPage() {
  ...
  const { data: classReminderStatus, isLoading: isLoadingClassReminders } = useQuery<ClassReminderStatus>({
    ...
    // Keeps Worker status reasonably fresh between page visits without
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="space-y-3">
        <div>
          <h2 className="text-lg font-semibold">Class Capacity</h2>
          <p className="text-sm text-muted-foreground">
      ...
      <section className="space-y-3">
        <div>
          <h2 className="text-lg font-semibold">Class Reminders</h2>
          <p className="text-sm text-muted-foreground">
