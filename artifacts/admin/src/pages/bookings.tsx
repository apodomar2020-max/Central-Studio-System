        ...
        <Table>
          <TableHeader>
            <TableRow className="border-border bg-muted/30 hover:bg-muted/30">
              <TableHead className="h-12 text-xs font-semibold text-muted-foreground">Participant</TableHead>
              <TableHead className="h-12 text-xs font-semibold text-muted-foreground">Account Owner</TableHead>
              <TableHead className="h-12 text-xs font-semibold text-muted-foreground">Scope</TableHead>
              <TableHead className="h-12 text-xs font-semibold text-muted-foreground">Class</TableHead>
              <TableHead className="h-12 text-xs font-semibold text-muted-foreground">Schedule</TableHead>
              <TableHead className="h-12 text-xs font-semibold text-muted-foreground">Booked</TableHead>
              <TableHead className="h-12 text-xs font-semibold text-muted-foreground">Booking Status</TableHead>
              <TableHead className="h-12 text-xs font-semibold text-muted-foreground">Payment Status</TableHead>
                  ...
                  <Fragment key={booking.id}>
                    <TableRow key={`${booking.id}-summary`} data-testid={`row-booking-${booking.id}`} className="border-b-0 bg-card hover:bg-muted/20">
                      <TableCell className="py-4">
                        <div className="flex items-center gap-3 border-r border-border/70 pr-4">
                          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-violet-300/40 bg-gradient-to-br from-violet-500/80 to-slate-700 text-sm font-semibold text-white shadow-[0_0_24px_rgba(139,92,246,0.18)]">
                      ...
                      <TableCell className="py-4">
                        <div className="border-r border-border/70 pr-4">
                          ...
                          <div className="max-w-[170px] break-words text-xs text-muted-foreground">{accountOwnerEmail(booking)}</div>
                        </div>
                      </TableCell>
                      <TableCell className="py-4">
                        <div className="flex border-r border-border/70 pr-4">
                          <span className="inline-flex min-w-[86px] items-center justify-center gap-2 rounded-md border border-cyan-500/50 bg-cyan-500/10 px-2.5 py-1.5 text-xs font-semibold text-foreground">
                      ...
                      <TableCell className="py-4">
                        <div className="flex items-center gap-3 border-r border-border/70 pr-4">
                          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-cyan-400/30 bg-cyan-500/15 text-cyan-300 shadow-[0_0_24px_rgba(0,182,215,0.16)]">
                      ...
                      <TableCell className="py-4">
                        <div className="flex items-center gap-3 border-r border-border/70 pr-4">
                          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-slate-600/60 bg-slate-700/40 text-blue-400">
                      ...
                      <TableCell className="py-4">
                        <div className="border-r border-border/70 pr-4 text-center">
                          <span className={`inline-flex min-w-[96px] items-center justify-center gap-2 rounded-md border px-2.5 py-1.5 text-xs font-semibold ${bookingStatusPillClass(bookingStatus)}`}>
                            <BookingStatusIcon className="h-4 w-4" />
                            {bookingStatusLabel(bookingStatus)}
                          </span>
                          <div className="mt-1 text-xs text-muted-foreground">Booking</div>
                        </div>
                      </TableCell>
                      <TableCell className="py-4">
                        <div className="border-r border-border/70 pr-4 text-center">
                          <span className={`inline-flex min-w-[132px] items-center justify-center gap-2 rounded-md border px-2.5 py-1.5 text-xs font-semibold ${paymentStatusPillClass(paymentStatus)}`}>
                            <PaymentStatusIcon className="h-4 w-4" />
                            {paymentStatusLabel(paymentStatus)}
                          </span>
                          <div className="mt-1 text-xs text-muted-foreground">Payment</div>
                        </div>
                      </TableCell>
                      <TableCell className="py-4 text-right">
                        {canEdit && (
                          <Button variant="outline" size="icon" className="h-9 w-9 text-muted-foreground" data-testid={`button-edit-booking-${booking.id}`} onClick={() => openEdit(booking)} title="Edit booking">
                    ...
                    {hasAnyActions && (
                      <TableRow key={`${booking.id}-actions`} className="border-b bg-card hover:bg-muted/20">
                        <TableCell colSpan={9} className="px-4 pb-4 pt-0">
                          <div className="rounded-lg border bg-background/50 p-3">
                            <div className={canShowBookingActions || canShowCancelAction ? canShowPaymentActions ? "grid gap-3 lg:grid-cols-2" : "grid gap-3" : "grid gap-3"}>
                              {(canShowBookingActions || canShowCancelAction) && (
                                <div className={`flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between ${canShowPaymentActions ? "lg:border-r lg:border-border/70 lg:pr-4" : ""}`}>
                              <div className="flex items-center gap-3">
                                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-emerald-500/25 bg-emerald-500/10 text-emerald-400">
                                  <CalendarCheck className="h-4 w-4" />
                                </div>
                                <div>
                                  <div className="text-sm font-semibold text-foreground">Booking Actions</div>
                              ...
                              <div className="flex flex-wrap gap-3 lg:justify-end">
                                {canShowBookingActions && (
                                  <>
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      className="h-9 min-w-[146px] border-emerald-500/60 bg-emerald-500/5 px-3 text-xs font-semibold text-emerald-400 hover:bg-emerald-500/10 hover:text-emerald-300"
                                    ...
                                    <Button
                                      variant="outline"
                                      size="sm"
                                      className="h-9 min-w-[138px] border-red-500/70 bg-red-500/5 px-3 text-xs font-semibold text-red-400 hover:bg-red-500/10 hover:text-red-300"
                                ...
                                {canShowCancelAction && (
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    className="h-9 min-w-[132px] border-orange-500/60 bg-orange-500/5 px-3 text-xs font-semibold text-orange-400 hover:bg-orange-500/10 hover:text-orange-300"
                              ...
                              {canShowPaymentActions && (
                                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between lg:pl-2">
                              <div className="flex items-center gap-3">
                                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-md border border-blue-500/25 bg-blue-500/10 text-blue-400">
                                  <CreditCard className="h-4 w-4" />
                                </div>
                                <div>
                                  <div className="text-sm font-semibold text-foreground">Payment Actions</div>
                              ...
                              <div className="flex flex-wrap gap-3 lg:justify-end">
                                <Button
                                  variant="outline"
                                  size="sm"
                                  className="h-9 min-w-[146px] border-blue-500/60 bg-blue-500/5 px-3 text-xs font-semibold text-blue-400 hover:bg-blue-500/10 hover:text-blue-300"
