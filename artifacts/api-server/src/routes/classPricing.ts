...
router.get("/admin/settings/class-pricing", requireAdminAuth, async (_req, res): Promise<void> => {
  const settings = await getOrCreateClassPricingSettings();
  res.json(settings);
});

router.patch("/admin/settings/class-pricing", requireAdminAuth, async (req, res): Promise<void> => {
