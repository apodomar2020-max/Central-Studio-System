...
export interface BalletApplicationPdfInput {
  ...
  currentPayment: Record<string, unknown> | null;
...
export async function buildBalletApplicationPdfBuffer(input: BalletApplicationPdfInput): Promise<Buffer> {
  ...
  section("Payment"); keyValues([["Package", asText(currentPayment?.packageName)], ["Amount", money(currentPayment?.amountEgp)], ["Billing Month", asText(currentPayment?.billingMonth)], ["Actual Method", titleCase(currentPayment?.paymentMethod)], ["Payment Status", titleCase(currentPayment?.status)], ["Payment Updated", formatDateTime(currentPayment?.updatedAt)]]);
  if (payments.length > 1) table([{ label: "Payment", width: 70 }, { label: "Package", width: 120 }, { label: "Amount", width: 70 }, { label: "Month", width: 70 }, { label: "Method", width: 80 }, { label: "Status", width: 70 }, { label: "Updated", width: 100 }], payments.map((p) => [`#${asText(p.id)}`, asText(p.packageName), money(p.amountEgp), asText(p.billingMonth), titleCase(p.paymentMethod), titleCase(p.status), formatDateTime(p.updatedAt)]));
