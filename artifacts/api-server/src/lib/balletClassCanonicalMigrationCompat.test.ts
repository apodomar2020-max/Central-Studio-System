/**
 * Verifies migration 0075 against a database shaped exactly like Production:
 * an already-populated legacy Ballet Class catalogue, not an empty one.
 *
 * This is the scenario the original (rejected) version of 0075 could not
 ...
 *      disposable Postgres database migrated through 0074 (NOT 0075) — i.e.
 *      including the reminder-automation migrations that now precede this
 *      one in the merged chain, but not yet the Ballet Class migration.
 ...
 *   3. Migration 0075 has then been applied on top of that seed (e.g. via
