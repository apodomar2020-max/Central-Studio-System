...
const migration = read("lib/db/migrations/0072_ballet_class_canonical_relationships.sql");
...
// ─── 0072 is a pure Expand migration ───────────────────────────────────────

test("0072 does not require an empty legacy catalogue", () => {
  assert.doesNotMatch(migration, /RAISE EXCEPTION/);
  assert.doesNotMatch(migration, /count\(\*\)\s*INTO/i);
});

test("0072 adds is_legacy defaulting true and nullable canonical relationships", () => {
...
test("0072 adds the composite Group/Level FK and the supporting unique constraint", () => {
...
test("0072 backfills level_id only for classes with exactly one legacy Level link, never a Group", () => {
  assert.match(migration, /HAVING count\(\*\) = 1/);
  assert.match(migration, /INNER JOIN "ballet_levels"/);
  assert.doesNotMatch(migration, /SET\s+"group_id"/i);
});

test("0072 does not drop any legacy relationship table", () => {
...
test("0072 defers the one-schedule-per-class unique index and Schedule shape CHECK constraints", () => {
